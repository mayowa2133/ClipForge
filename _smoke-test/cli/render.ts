...
function renderWithConcatDemuxer(
  ...
  const tmpDir = join(dirname(config.output_path), ".render-tmp");
  ...
  for (let i = 0; i < segments.length; i++) {
    ...
    try {
      ...
      console.error(`    ERROR extracting segment ${i}: ${e.message?.slice(0, 200)}`);
      ...
      execSync(fallbackCmd, { stdio: ["pipe", "pipe", "pipe"], timeout: 120_000 });
    }
  ...
  const concatDurationS = segments.reduce((sum, segment) => sum + segment.duration_s, 0);
  const speed =
    config.target_duration_s && config.target_duration_s > 0 && concatDurationS > 0
      ? concatDurationS / config.target_duration_s
      : 1;
  const videoPaceFilter = Math.abs(speed - 1) > 0.001
  ...
  const filterComplex = [
    ...
    `[1:a]volume=${config.music_volume}[musicvol]`,
